...
_material_name_to_pole_residue = {
...
def get_medium(spec: MaterialSpecTidy3d) -> td.Medium:
    ...
    if isinstance(spec, int | float):
        ...
        return spec
    elif spec in _material_name_to_pole_residue:
        return _material_name_to_pole_residue[spec]
    elif isinstance(spec, PoleResidue):
        return spec
    elif (
        spec in td.material_library
        or isinstance(spec, tuple)
        and tuple[0] in td.material_library
    ):
        if isinstance(spec, str):
            ...
            return td.material_library[spec[0]].variant
