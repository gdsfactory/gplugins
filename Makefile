...
install:
	uv venv --python 3.11
	uv pip install -e .[dev,docs,emode,femwell,meow,sax,tidy3d,klayout,vlsir]
...
uv-test:
	@for plugin in emode femwell meow sax tidy3d klayout vlsir path_length_analysis; do \
		uv run pytest gplugins/$$plugin; \
	done

cov:
	uv run pytest gplugins/emode gplugins/femwell gplugins/meshwell gplugins/meow gplugins/sax gplugins/tidy3d gplugins/klayout gplugins/vlsir gplugins/path_length_analysis --cov=gplugins/emode --cov=gplugins/femwell --cov=gplugins/meshwell --cov=gplugins/meow --cov=gplugins/sax --cov=gplugins/tidy3d --cov=gplugins/klayout --cov=gplugins/vlsir --cov=gplugins/path_length_analysis
